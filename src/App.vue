...
export default {
  ...
  methods: {
    ...
    getUser(id, name) {
      ...
      this.status.userName = name
    },

    getTasks(items, permanent) {
      this.tasks = []
      if (permanent) document.getElementById('taskslist').scrollTop = 0
      let username = this.persons[this.status.person].name
    ...
    getProjects(items) {
      ...
      for (let id in this.projects) {
        if (this.projects[id].name == 'Inbox') this.status.inboxId = id
      }
      //console.log('Inbox', this.status.inboxId)
    },

    getNotes(items) {
      this.notes = []
      for (let item in items) this.notes.push(items[item])
